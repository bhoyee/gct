...
$query= "Select SUM(price) as amount, EXTRACT(MONTH From `payDate`) AS Mon, EXTRACT(YEAR From `payDate`) AS Year from invoice where status='Paid' AND EXTRACT(YEAR From `payDate`)=$year GROUP BY EXTRACT(MONTH From `payDate`), EXTRACT(YEAR From `payDate`);";
 ...
 $result = mysqli_query($conn, $query);  
    ...
    $unpaid = 'Unpaid'; 
     ...
     $total_sale = number_format($tSale, 2);
     ...
     if (is_null($total_sale)){
        ...
        $total_sale = 0;
         ...
         $mSales = $monthSale['price'];
         $mSale = number_format($mSales, 2);
         if (is_null($mSale)){
            $mSale = 0;
         }
     ...
     $tuSales = $totaluSale['price'];
     $tuSale = number_format($tuSales, 2);
     if (is_null($tuSale)){
        $tuSale = 0;
     }
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex">
                                            <div class="flex-grow-1">
                                                ...
                                                <h4 class="mb-2">$<?php echo $total_sale; ?></h4>
                                            </div>
                                            <div class="avatar-sm">
...
<script type="text/javascript">
    ...
    google.charts.setOnLoadCallback(drawChart);
        ...
        function drawChart() {
        <?php
        // Check if $rows is empty or not
        if (empty($rows)) {
            // If $rows is empty, display "No data"
        ?>
        var data = google.visualization.arrayToDataTable([['Trip Character', 'Number of Trips'], ['No data', 0]]);
        <?php
        } else {
            // If $rows is not empty, construct the data array normally
        ?>
        var data = google.visualization.arrayToDataTable([
            ['Trip Character', 'Number of Trips'],  // Column labels
            <?php
            foreach($rows as $row) {
                $tripChar = addslashes($row["tripChar"]);  // X-axis: Trip Character (String)
                $num = (int)$row["num"];  // Y-axis: Number of Trips (Integer)
                echo "['$tripChar', $num],";  // Ensure proper format
            }
            ?>
        ]);
        <?php } ?>
        ...
        var options = {
            title: 'Trip Information for <?php echo $year; ?>',
            width: "100%",
            height: 400,
            bar: {groupWidth: "45%"},
