...
if (!isset($_POST['save']) || $_POST['save'] != 'contact') {
	$script = "<script>
	window.location = 't3458jdey8est.php';</script>";
	echo $script;
	exit();
}
...
$_SESSION['userid'] = $user_id;
 ...
 $seatNumber = NULL;
	...
	for($i=1; $i<$st; $i++)
	{
		...
		{
			$queryseatz = "INSERT INTO seats(bookingCode, number, PNR, date, route, bus, status) VALUES ('". $bookID ."', '" . intval($i) . "', '". $calcPNR ."', '". $doj ."','". $route ."', '". $pcar ."','active')";
	//		$results = mysql_real_escape_string($query);
			...
			$seatNumber = $nseat;
		      ...
		      $status = 'active';
			   ...
			   $sql13_cus = "INSERT INTO book (bookingCode, route, pCar, bookdate, returnD, time, seat, tripChar, email, regDate, bookstatus, price, paystatus, userid) VALUES ('". $bookID ."','" . $route . "', '" . $pcar ."','" . $dptDate . "','" . $rdate . "','" . $dptTime . "','" . $seatNumber . "','" . $tripChar . "','" . $email . "','" . $regDate . "','" .$status."',".$price.", 'UnPaid','" . $_SESSION['userid']. "')";
	...
	for($i=1; $i<9; $i++)
	{
		...
		if( !empty($_POST[$chparam]) )
		{
			$queryseat = "INSERT INTO seats(bookingCode, number, PNR, date, route, status) VALUES ('". $bookID ."', '" . intval($i) . "', '". $calcPNR ."', '". $doj ."','". $route ."', 'active')";
	...
	//		$results = mysql_real_escape_string($query);
		   ...
		   $status = 'active';
			   ...
			   //   echo "<h3>". strtoupper($str_arr[0] .' '.'=&gt;'.' '. $str_arr[1]) ."<small>".' '.' '."</small></h>";
			   $sql3_cus = "INSERT INTO book (bookingCode, route, bookdate,	returnD, time, seat, tripChar, email, regDate, bookstatus, price, paystatus, userid) VALUES ('" . $bookID . "','" . $route . "','" . $dptDate . "','" . $rdate . "','" . $dptTime . "','" . $seatNumber . "', '" . $tripChar . "', '" . $email . "','" . $regDate . "','" . $status . "','" . $price . "','UnPaid','" . $userID . "')";
